...
earth = Planet('earth', [earthimg, cloudimg], shader, 0.125, 80000000, pos=[0.0, -1.0, -9.0], 
            vel=[0.0, 0.0, 0.0], track_shader=None, light=sunlight)
moon = Planet('moon', [moonimg], shader, 0.025, 80000000, pos=[0.0, -1.0, -9.6], 
            vel=[0.22, 0.044, 0.0], track_shader=None, light=sunlight)
...
# Display scene
time_step = 0
f = pi3d.Font("fonts/FreeSans.ttf", (0x88, 0, 0, 255))
s = pi3d.String(font=f, string="")
s.translate(-0.5, -0.5, -9.0)
s.set_shader(flatsh)
while DISPLAY.loop_running():
  if rottilt and changerot:
    CAMERA.relocate(rot, tilt, [0.0, -1.0, -8.5], camRad)
    rottilt = False
    changerot = False
  if time_step % 1 == 0:
    moon.pull([earth])
    asteroid.pull([earth])
    asteroid.collide(earth)
    time_step = 0
  time_step += 1
  ...
  myecube.draw()
  # Check for collusion
  if asteroid.collision_detection(earth) and False:
    # Freeze 
    # zoom in on earth
  ...
  if False and rot != (mx * -0.1) or tilt != (my * 0.1):
    rot = mx * -0.1
    tilt = my * 0.1
    rottilt = True
  ...
  if k >-1:
    ...
    if k==112:
      ...
      s = pi3d.String(font=f, string="%r" % (asteroid.pos))
      s.translate(-0.5, -0.5, -9.0)
      s.set_shader(flatsh)
